<h1 align="center">Hi 👋, I'm Umair</h1>
<h3 align="center">Aspiring Front-End Web Developer from India 🚀</h3>

<p align="center">
  <a href="https://umair-webdev.github.io/MyPortfoilio/">
    <img src="https://img.shields.io/badge/Portfolio-Visit%20My%20Website-0891B2?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Portfolio" />
  </a>
  <a href="mailto:uk306927@gmail.com">
    <img src="https://img.shields.io/badge/Email-Contact%20Me-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>
</p>

---

## 👨‍💻 About Me

- 🎓 I'm currently pursuing **12th Science** with **Information Technology (IT)**
- 💻 I build responsive websites using **HTML, CSS & JavaScript**
- 🌐 I'm focused on growing as a **Front-End Web Developer**
- 🐍 Currently exploring **Python Programming**
- 🐧 Learning **Linux & Kali Linux**
- 🔐 Interested in **Cybersecurity & Ethical Hacking**
- 🚀 I enjoy turning ideas into practical web projects
- 📚 Always learning, building and improving my technical skills

---

## 🛠️ Tech Stack

<p align="left">
  <img src="https://skillicons.dev/icons?i=html,css,js,python,git,github,firebase,linux" alt="Tech Stack" />
</p>

### Currently Exploring

`Python` • `Linux` • `Kali Linux` • `Cybersecurity`

---

## 🚀 Featured Projects

### 🍽️ Nahid Caterers — Quotation Generator
A web-based catering quotation system focused on quotation creation, management, mobile responsiveness, Firebase integration, cloud features and PWA functionality.

**Tech:** `HTML` • `CSS` • `JavaScript` • `Firebase` • `PWA`

### 🏗️ Sai Darshan Construction — Business Website
A responsive business website concept designed to showcase company information and provide customer contact functionality.

**Tech:** `HTML` • `CSS` • `JavaScript` • `Responsive Design`

### 🌐 Personal Portfolio
My personal developer portfolio showcasing my skills, projects and journey in web development.

👉 [**View Live Portfolio**](https://umair-webdev.github.io/MyPortfoilio/)

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=umair-webdev&show_icons=true&theme=tokyonight&hide_border=true" alt="Umair's GitHub Stats" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=umair-webdev&layout=compact&theme=tokyonight&hide_border=true" alt="Top Languages" />
</p>

---

## 🎯 My Goals

My goal is to start my career in **Web Development**, gain real-world industry experience and continuously improve my programming skills. I also want to explore advanced areas of **Cybersecurity and Ethical Hacking** in the future.

---

## 🤝 Connect With Me

- 🌐 **Portfolio:** [umair-webdev.github.io/MyPortfoilio](https://umair-webdev.github.io/MyPortfoilio/)
- 💻 **GitHub:** [@umair-webdev](https://github.com/umair-webdev)
- 📧 **Email:** uk306927@gmail.com

---

<p align="center">
  <b>⚡ Keep Learning. Keep Building. Keep Growing. 🚀</b>
</p>

<p align="center">Thanks for visiting my GitHub profile! ⭐</p>
